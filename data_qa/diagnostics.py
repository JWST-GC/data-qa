...
def _caption_for_impl(n, metrics):
    ...
    if n == 4:
        ...
        base = (f"**Stage 4 — positional offsets (JWST catalogue − VIRAC).** How far a star in the "
                ...
                f"LEFT maps that offset across the mosaic. Outlined cells are adjacent cells that "
                f"each deviate from the field value — a coherent block, which is the signature of "
                f"a sub-region registered differently from the rest of the field. A lone deviating "
                f"cell is a mis-measurement in that one cell and stays unoutlined. Grey cells were "
                f"not measurable.\n\n"
