...
_WORKFLOW_OPTIONS = ["Data taken", "QA assigned / under examination", "Needs discussion",
                     "Needs attention", "Done"]
# SCRIPT-OWNED review lifecycle: a coarse "is the pipeline + QA finished?" tag, rewritten every
# run.  In progress = at least one pipeline/QA stage is still incomplete; Ready for review = every
# expected stage has run (whether it came back clean or flagged -- a red flag is a finding for a
# human to review, not an unfinished stage).  MIRI-only and meta cards get no review status.
_REVIEW_OPTIONS = ["In progress", "Ready for review"]
_METRICS_DIR = os.environ.get("QA_METRICS_DIR") or os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "data_qa", "metrics"))


def _review_status(cat):
    """Review-lifecycle option for a Measured category, or None when it does not apply."""
    if cat in ("incomplete", "nometrics"):
        return "In progress"
    if cat in ("clean", "attention", "redflag", "error"):
        return "Ready for review"
    return None                                           # MIRI / meta: no review status


def _token_for(args):
    """Route each gh call to a token with the right scope: ``gh issue`` / ``gh api repos`` need
    repo scope; ``gh project`` / ``gh api graphql`` need Projects scope.  ``QA_REPO_TOKEN`` /
    ``QA_PROJECT_TOKEN`` override per class; both fall back to ``GH_TOKEN`` / ``GITHUB_TOKEN`` (a
    single token that carries both scopes)."""
    both = os.environ.get("GH_TOKEN") or os.environ.get("GITHUB_TOKEN")
    if args[:1] == ("project",) or args[:2] == ("api", "graphql"):
        return os.environ.get("QA_PROJECT_TOKEN") or both
    return os.environ.get("QA_REPO_TOKEN") or both
...
def _gh(*args, check=True):
    ...
    if tok:
        env["GH_TOKEN"] = tok
        env.pop("GITHUB_TOKEN", None)                     # GH_TOKEN wins; keep the routing clean
...
def main(argv=None):
    ...
    if not (os.environ.get("GH_TOKEN") or os.environ.get("GITHUB_TOKEN")
            or (os.environ.get("QA_REPO_TOKEN") and os.environ.get("QA_PROJECT_TOKEN"))):
    ...
    frev = _ensure_single_select(args.owner, pnum, "Review status", _REVIEW_OPTIONS, fields, apply)
    ...
    for r in rows:
        ...
        want_rev = _review_status(r["cat"])
        cur_item = existing.get(r["num"])
        old = _item_field(cur_item, "Measured") if cur_item else None
        old_rev = _item_field(cur_item, "Review status") if cur_item else None
        if old != want:
            changed += 1
            print(f"#{r['num']:>3}  Measured: {old or '—'} -> {want}")
        if want_rev and old_rev != want_rev:
            print(f"#{r['num']:>3}  Review status: {old_rev or '—'} -> {want_rev}")
        if not apply:
            continue
        ...
        if frev and want_rev in rev_opt:
            _gh("project", "item-edit", "--id", iid, "--project-id", pid,
                "--field-id", frev["id"], "--single-select-option-id", rev_opt[want_rev])
        if fstages:
            _gh("project", "item-edit", "--id", iid, "--project-id", pid,
